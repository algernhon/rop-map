(function () {
	...
	RS.prototype.createScale = function (resize) {
		this.step = Math.floor(this.sliderWidth / (this.conf.values.length - 1));
		...
		for (var i = 0, iLen = this.conf.values.length; i < iLen; i++) {
			...
			if (!this.conf.labels) {
				if (i === 0 || i === iLen - 1) ins.innerHTML = this.conf.show[i]
			}
			else ins.innerHTML = this.conf.show[i];
	...
	RS.prototype.setValues = function (start, end) {
		...
		if (this.conf.range) {
			...
			this.input = [this.conf.values[this.values.start], this.conf.values[this.values.end]];
	...
	RS.prototype.onChange = function () {
		...
		this.timeout = setTimeout(function () {
			if (_this.conf.onChange && typeof _this.conf.onChange === 'function') {			
				return _this.conf.onChange(_this.input);
			}
		}, 100);
	};
